#!/usr/bin/env python3
"""
Django management command to import data from extracted Laravel databases
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        qa_file = options['qa_file']
        skip_existing = options['skip_existing']

        if not os.path.exists(qa_file):
            self.stdout.write(self.style.ERROR(f"File not found: {qa_file}"))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f"Loaded {len(posts)} records from {qa_file}"))
